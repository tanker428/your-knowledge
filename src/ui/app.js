...
export async function initApp(deps) {
  ...
  function renderQuizPlacementBoard(/** @type {any} */ quiz, /** @type {string|null} */ selectedReferenceId, /** @type {boolean} */ answered) {
    ...
    if (quiz.questionType !== "timeline-map") {
      return `<div class="quiz-choice-board" aria-label="候補一覧">${options.map((option) => `<button class="quiz-placement quiz-choice-option ${selectedReferenceId === option.id ? (answered ? "correct" : "selected") : ""}" data-quiz-drop="${escapeHtml(option.id)}" ${answered ? "disabled" : ""}>${option.photoId ? renderQuizImage(photoById(option.photoId), option.region, option.label, "quiz-choice-media") : ""}<span>${escapeHtml(option.label)}</span></button>`).join("")}</div>`;
    }
  ...
  function renderQuizImage(/** @type {any} */ photo, /** @type {any} */ region, /** @type {string} */ alt, /** @type {string} */ className) {
    const source = escapeHtml(photo?.src || MISSING_PHOTO_SRC);
    const safeAlt = escapeHtml(alt || "写真");
    if (!region || !Number(region.w) || !Number(region.h)) return `<span class="${className}"><img src="${source}" alt="${safeAlt}" /></span>`;
    const x = Number(region.x) || 0;
    const y = Number(region.y) || 0;
    const width = Number(region.w);
    const height = Number(region.h);
    return `<span class="${className} quiz-region-crop" style="--crop-x:${x / 100};--crop-y:${y / 100};--crop-w:${width / 100};--crop-h:${height / 100}"><img src="${source}" alt="${safeAlt}" /><i aria-hidden="true"></i></span>`;
  }
