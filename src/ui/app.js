...
export async function initApp(deps) {
  ...
  function renderQuizPlacementBoard(/** @type {any} */ quiz, /** @type {string|null} */ selectedReferenceId, /** @type {boolean} */ answered) {
    ...
    if (quiz.questionType !== "timeline-map") {
      return `<div class="quiz-choice-board" aria-label="候補一覧">${options.map((option) => `<button class="quiz-placement quiz-choice-option ${selectedReferenceId === option.id ? (answered ? "correct" : "selected") : ""}" data-quiz-drop="${escapeHtml(option.id)}" ${answered ? "disabled" : ""}>${option.photoId ? `<span class="quiz-choice-media"><img src="${escapeHtml(photoById(option.photoId)?.src || MISSING_PHOTO_SRC)}" alt="" /></span>` : ""}<span>${escapeHtml(option.label)}</span></button>`).join("")}</div>`;
    }
